...
type config struct {
	App struct {
		Environment string `env:"USER_APP_ENV"   envDefault:"dev"`
		LogLevel    string `env:"USER_LOG_LEVEL" envDefault:"INFO"`
	}
	HTTP struct {
		Port int `env:"USER_HTTP_PORT" envDefault:"4000"`
	}
	GORM struct {
		JsonLogger bool `env:"USER_GORM_JSON_LOGGER" envDefault:"false"`
	}
	POSTGRES struct {
		...
		Database string `env:"USER_POSTGRES_DATABASE" envDefault:"USER"`
		Sslmode  string `env:"USER_POSTGRES_SSLMODE"  envDefault:"disable"`
	}
	REDIS struct {
		Host string `env:"USER_REDIS_HOST" envDefault:"localhost:6379"`
		DB   int    `env:"USER_REDIS_DB"   envDefault:"0"`
	}
}
